...
**Zur Zeile „Build/Packaging".** Diese Entscheidung ist **getroffen** und wird hier als **Decision-Disposition** geführt. Sie wird — wie die `P-3`-, die Sprach-/Runtime-, die Source-Tree-, die Dependency-Admission-, die `NEW-8`- und die R6-Zeile — vom bestehenden Gate-A-Punkt `A-11` (§16.1) **erfasst**, erfüllt ihn aber **nicht**. Ob und wie die Build-/Packaging-, Reproduzierbarkeits- und Toolchain-Governance-Entscheidung dauerhaft ADR-seitig eingelöst wird, ist hier **nicht** entschieden: es entsteht **keine** ADR-Datei, **keine** ADR-Nummer, **kein** ADR-Kandidat, **keine** Decision-ID, **keine** `HM-*`-Kennung, **keine** Zeile im Decision Index oder Risk Register und **kein** Work Package. Es existieren weiterhin **0** ADR-Dateien und **0** akzeptierte ADRs.

**`A-11` bleibt `offen`.** `A-11` verlangt die Disposition **der Punkte dieser §14**. Alle sieben Zeilen (`P-3`, die R6-Zusammensetzung, Sprache/Runtime, Source Tree, Dependency-Admission, `NEW-8` und Build/Packaging) sind inzwischen **entschieden** — das erfüllt `A-11` gleichwohl **nicht**: `A-11` verlangt zusätzlich eine **eigene, ausdrückliche Human-Maintainer-Disposition** der ADR-/Decision-Ebene, und es bleibt unverändert offen, **wie** die für mehrere dieser Zeilen dokumentierte ADR-Pflicht dauerhaft eingelöst wird. `A-11` ist bis zu dieser gesonderten Entscheidung **offen**; akzeptierte ADRs bleiben **0** und der Decision Index bleibt **unverändert**.
...
**Bilanz Gate A: 12 von 14 erfüllt, 0 teilweise, 2 offen. Gate A ist nicht passiert.** Offen bleiben genau zwei Punkte: `A-11` und `A-14`.
