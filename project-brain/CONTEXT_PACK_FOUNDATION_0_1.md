...
- **Phase:** `Foundation 0.1` (interner Phasenname; Release-Taxonomie `Accepted` via `HM-1`: Foundation-Tag-**Kandidat** `v0.0.1-foundation`, erster Observe-Prerelease-**Kandidat** `v0.1.0-alpha.1` — beide sind Kandidaten; **durch `CO-WP-031` bzw. die Post-Integration-Reconciliation wurde kein Tag erzeugt und keine Tag-Erzeugung autorisiert**; jede spätere Tag- oder Release-Aktion ist ein eigenes Human-Maintainer-Autoritätsereignis).
